# commit-message: chore: update About metadata for V2.8
from pathlib import Path

path = Path('lib/features/editor/spell_checker_page.dart')
text = path.read_text()
old_version = "applicationVersion: '2.7.0'"
new_version = "applicationVersion: '2.8.0'"
...
for marker in (old_version, old_description):
    if text.count(marker) != 1:
        raise SystemExit(f'Expected exactly one About marker: {marker!r}')

path.write_text(text.replace(old_version, new_version).replace(old_description, new_description))
