...
import 'package:spellchecker/writing/rules/unmatched_curly_brace_rule.dart';
...
void main() {
  ...
  group('UnmatchedCurlyBraceRule', () {
    const rule = UnmatchedCurlyBraceRule();

    test('accepts balanced and nested curly braces', () {
    ...
    test('reports an unmatched opening curly brace as advisory-only', () {
      const text = 'Keep {this text';
      final issue = rule.analyze(text, us).single;

      expect(issue.ruleId, 'unmatched-curly-brace');
      expect(issue.ruleName, 'Unmatched curly brace');
      expect(issue.message, contains('opening curly brace'));
    ...
    test('reports an unmatched closing curly brace as advisory-only', () {
      const text = 'Keep this} text';
      final issue = rule.analyze(text, us).single;

      expect(issue.message, contains('closing curly brace'));
      expect(issue.originalText, '}');
    ...
    test('does not claim parenthesis or square bracket ownership', () {
