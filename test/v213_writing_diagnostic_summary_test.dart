...
void main() {
  test('diagnostic summary includes unmatched parenthesis metadata only', () {
    ...
    final rule = summary.rules.singleWhere(
      (rule) => rule.ruleId == 'unmatched-parenthesis',
    );
    expect(rule.displayName, 'Unmatched parenthesis');
    expect(rule.capturedIssueCount, 1);
    expect(rule.totalIssueCount, 1);
    expect(
      output,
      contains(
        '- Unmatched parenthesis [unmatched-parenthesis]: 1/1 captured/total',
      ),
    );
  ...
  test('clean diagnostics materialize a zero total for the eighth rule', () {
    final analyzer = WritingAnalyzer();
    final result = analyzer.analyze(
      'Balanced (text).',
      languagePack: SpellLanguageRegistry.englishUs,
