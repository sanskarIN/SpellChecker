...
import 'package:spellchecker/writing/rules/unmatched_square_bracket_rule.dart';
...
void main() {
  ...
  group('UnmatchedSquareBracketRule', () {
    const rule = UnmatchedSquareBracketRule();

    test('accepts balanced and nested square brackets', () {
    ...
    test('reports an unmatched opening square bracket as advisory-only', () {
      const text = 'Keep [this text';
      final issue = rule.analyze(text, us).single;

      expect(issue.ruleId, 'unmatched-square-bracket');
      expect(issue.ruleName, 'Unmatched square bracket');
      expect(issue.message, contains('opening square bracket'));
    ...
    test('reports an unmatched closing square bracket as advisory-only', () {
      const text = 'Keep this] text';
      final issue = rule.analyze(text, us).single;

      expect(issue.message, contains('closing square bracket'));
      expect(issue.originalText, ']');
    ...
    test('does not claim parenthesis or curly bracket ownership', () {
